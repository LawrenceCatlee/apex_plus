...
def main(args: argparse.Namespace):
    ...
    prefill_cluster = Cluster.from_gpu(args.p_gpu, args.num_p_nodes, args.num_p_gpus_per_node)
    decode_cluster = Cluster.from_gpu(args.d_gpu, args.num_d_nodes, args.num_d_gpus_per_node)
    ...
    engine = SearchEngine(
        model, prefill_cluster, trace, "prefill", dtype
    )  # search for prefill cluster
    ...
    engine = SearchEngine(
        model, decode_cluster, trace, "decode", dtype
    )  # search for decode cluster
    _, trace = engine.search(
        args.all,
        args.frequency,
        args.request_percentiles,
...
if __name__ == "__main__":
    ...
    # Prefill cluster config
    parser.add_argument(
        "--num-p-nodes", type=int, default=1, help="Number of prefill nodes in the cluster."
    )
    parser.add_argument(
        "--num-p-gpus-per-node",
        type=int,
        required=True,
        help=" Number of GPUs per node in the prefill cluster",
    )
    parser.add_argument(
        "--p-gpu",
        type=str,
    ...
    # Decode cluster config
    parser.add_argument(
        "--num-d-nodes", type=int, default=1, help="Number of decode nodes in the cluster."
    )
    parser.add_argument(
        "--num-d-gpus-per-node",
        type=int,
        required=True,
        help=" Number of GPUs per node in the decode cluster",
    )
    parser.add_argument(
        "--d-gpu",
        type=str,
